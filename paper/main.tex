...
Extend the resolution mix of Theorem~\ref{thm:stability} with a fraction $u$ of \emph{undetected} fraud, whose principal remains in circulation while adding no value: fractions $r$ (repaid), $\delta_g$ (good faith), $\delta_f$ (detected and remediated), $u$, with $r+\delta_g+\delta_f+u=1$. With $P_{\mathrm{net}}=(r+\delta_g)\,C_{\mathrm{tot}}$,
\[
...
\begin{proof}
Undetected fraud contributes its full principal $1$ per unit and nothing to $P_{\mathrm{net}}$, so $\Supply=C_{\mathrm{tot}}\bigl[r\varepsilon+\delta_g+u\bigr]$. Splitting the ratio, $(r\varepsilon+\delta_g)/(r+\delta_g)\le\max(1,\varepsilon)$ as in Theorem~\ref{thm:stability}, and the residual term is $u/(r+\delta_g)=u/(1-\delta_f-u)$.
\end{proof}
...
\begin{description}[leftmargin=*,itemsep=3pt]
\item[Bounded inflation.] Keep $\varepsilon\le K^*-1$ via \eqref{eq:epsilon}, so $\Supply/P_{\mathrm{net}}\le\max(1,\varepsilon)\le K^*$ (Theorem~\ref{thm:stability}; Proposition~\ref{prop:genstab} quantifies the degradation from undetected fraud). A smaller $K^*$ tightens the money-to-value ratio. Default $K^*=1.5$.
