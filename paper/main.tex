...
The results below strengthen Theorem~\ref{thm:stability} by removing its remediation hypothesis, characterize the long-run price, show that the discounted-withdrawal rule cannot exhaust the fund except in the limit of complete exit, and give a sufficient condition under which fraud is unprofitable.
...
\subsection{Unprofitability of fraud (a sufficient condition)}

\begin{proposition}[Fraud unprofitability]\label{prop:fraud}
Consider a fraudulent cluster that nets $W>0$ if it converts extracted value to USDC before detection, but forfeits stake $L>0$ (burned balances and reputation) if detected first. Suppose detection occurs independently each period with probability at least $p$, and new accounts cannot convert during a lock-up of $\tau_{\mathrm{lock}}$ periods. Then the expected profit satisfies
\[
\mathbb{E}[\pi]\ \le\ (1-p)^{\tau_{\mathrm{lock}}}\,W-\bigl(1-(1-p)^{\tau_{\mathrm{lock}}}\bigr)L,
\]
which is non-positive whenever $(1-p)^{\tau_{\mathrm{lock}}}\le L/(W+L)$.
\end{proposition}

\begin{proof}
No conversion is possible before $\tau_{\mathrm{lock}}$, so extracting $W$ requires surviving undetected for at least $\tau_{\mathrm{lock}}$ periods, an event of probability at most $(1-p)^{\tau_{\mathrm{lock}}}$. Bounding the gain by $W$ on that event and the loss by $L$ on its complement gives the inequality; requiring $\mathbb{E}[\pi]\le0$ and solving yields $(1-p)^{\tau_{\mathrm{lock}}}\le L/(W+L)$.
\end{proof}

\begin{remark}
With the default lock-up ($\tau_{\mathrm{lock}}=60$ days) and any non-negligible per-period detection hazard from the $3\%$ stochastic audit and the concentration monitoring of Section~\ref{sec:confidence}, the threshold holds for realistic $W/L$: the lock-up forces the attacker to hold an unbacked, detectable position long enough that detection dominates. A full game-theoretic optimum over adaptive strategies remains open (Section~\ref{sec:limitations}).
\end{remark}
