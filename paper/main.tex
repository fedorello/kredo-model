...
\VerifiedValue(t) = \sum_{\tau \in H_t}\bigl(1+\varepsilon(\tau)\bigr)C(\tau) \;-\; \mathrm{Burned}(t),
\]
where $\varepsilon(\tau)$ is the compensation coefficient of \eqref{eq:epsilon} fixed at the time of $\tau$ and $\mathrm{Burned}(t)$ is the cumulative amount removed by defaults, penalties, and conversions. Since $\Supply$ contains both issued principal and escrowed compensation net of burns, $\VerifiedValue$ is defined to match it; any residual gap in (I3) reflects only the lag between fraud and its detection.
...
\section{Emission and the compensation coefficient}

\subsection{Mechanics}

In \textsc{Transact}, the credit portion is $N_{\mathrm{credit}} = \max\bigl(0,\ N - \max(0,b_B)\bigr)$; the provider is credited $N$, and $B$'s balance decreases by $N$, opening a debt. Simultaneously a \emph{compensatory emission} enters escrow,
...
\begin{remark}
Theorem~\ref{thm:stability} is the inflation brake: as realized defaults rise, $\varepsilon$ falls automatically, and at $\delta = 0.30$ compensation is disabled entirely (strict conservation). The system does not assume full repayment; it bounds the money-to-value ratio for any $\delta \le \tfrac12$.
\end{remark}
...
Price is stationary when supply growth matches asset growth, $\dot S/S = \dot A/A$ with $A = F + \mu\,\ExtRev$. This partitions dynamics into a growth regime, a stable regime, and a falling regime.
...
\begin{proof}[Sketch]
Stationarity $\dot A / A = \dot S / S$ with $\dot A = \lambda_{\mathrm{inv}} + \mu\lambda_{\mathrm{rev}}$ and $\dot S = \mathrm{EmissionRate}$ (net of burns) gives, for a non-negative price derivative, $\lambda_{\mathrm{inv}} + \mu\lambda_{\mathrm{rev}} \ge \mathrm{EmissionRate}\cdot P$; solving for $\lambda_{\mathrm{rev}}$ yields the stated bound.
\end{proof}
...
\bibitem{buterin2019} V.~Buterin, Z.~Hitzig, and E.~G.~Weyl. A flexible design for funding public goods. \emph{Management Science}, 65(11):5171--5187, 2019.
