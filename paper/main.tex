...
With a per-day cluster-detection probability $p=0.05$ acting over the amount-scaled vesting (the base lock-up held non-detecting, matching the conservative audit-only baseline), the bound gives $w\approx0.053$ days per V---about $5.3$ days of extra vesting per $100~\V$. The simulator's \texttt{FraudDeterrenceService} then drives the optimum down through the profitable baseline: $+425\to+67\to0~\V$ at $w=0,\,0.02,\,0.053$, so the fraud is non-positive (deterred) at the unchanged $\Lambda=300~\V$. Escalating a flagged cluster's audit to $a=0.30$ is even sharper, giving $1/\beta\approx140~\V<\Lambda$ outright. A Phase-6 experiment closes the loop end-to-end: an actual wash-trading cluster is run through the engine's real \textsc{Transact}, \textsc{Convert}, and fraud-remediation operations with detection drawn stochastically per trade and per lock-up day. Over $30$ seeds the baseline fraud is profitable at every size ($+233$ to $+307~\V$ realised), each v2 lever \emph{on its own}---the dynamic lock-up alone (the vesting gate holds the position while the hazard accrues) and the flagged audit alone---drives the realised profit negative, and the empirical $\mathbb E[\pi]$ tracks the closed-form bound within Monte-Carlo error.
