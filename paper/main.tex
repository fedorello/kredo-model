...
The seven invariants below formalize Axioms~\ref{ax:create}--\ref{ax:power}. Write $\Supply(t) = \sum_{m} \max(0,b_t(m)) + E_t + G_t$ and $\VerifiedValue(t) = \sum_{\tau \in H_t}\, N_{\mathrm{credit}}(\tau)\,\confidence(\tau)$, where $N_{\mathrm{credit}}(\tau)$ is the newly issued (credit) portion of transaction $\tau$ and $\confidence(\tau)\in[0,1]$ is defined in Section~\ref{sec:confidence}.
...
\end{equation}
Thus $s_a = 0.61,\,0.14,\,0.01$ at $z = 2,3,4$. The \emph{review-score} $s_r$ is the median vote of randomly drawn reviewers (robust to outliers), and the \emph{audit-score} $s_x$ is $1$ unless a stochastic audit finds a violation. Review is triggered when $|z|>1$, when $N$ exceeds the auto-approval limit $N_{\mathrm{auto}} = 50\sqrt{r_A r_B + 1}$, when a party is under monitoring, or when the $3\%$ random audit selects $\tau$.
...
\end{equation}
The escrow is released pro rata upon repayment and \emph{burned} upon default. Released amounts are distributed across members in proportion to active turnover $\Turnover(m,t) = \sum_{\tau: t-\tau<90d,\, m\in\tau} N(\tau)\confidence(\tau)$,
\begin{equation}
...
Consider a stationary regime with constant default rate $\delta \le \tfrac12$ and $\varepsilon$ given by \eqref{eq:epsilon}. Let $P_{\mathrm{net}} = \VerifiedValue \cdot (1-\delta)$ denote net verified value. Then
\[
...
\begin{proof}
In a stationary regime with total issuance $N_{\mathrm{tot}}$, defaulted credits and their compensation are burned, so the surviving supply is $\Supply = N_{\mathrm{tot}}(1+\varepsilon)(1-\delta)$ while $P_{\mathrm{net}} = N_{\mathrm{tot}}(1-\delta)$. Hence $\Supply/P_{\mathrm{net}} = 1 + \varepsilon$. By \eqref{eq:epsilon}, $\varepsilon \le K^*-1$, giving $\Supply/P_{\mathrm{net}} \le K^*$. For $\delta > \delta^*$ the term $-\kappa(\delta-\delta^*)$ makes $\varepsilon$ strictly smaller, tightening the bound.
\end{proof}
...
\textsc{Invest} (deposit $\Delta U$ of USDC for $\Delta V = \Delta U/P_t$ of $\V$) and \textsc{Convert} (sell $\Delta V$ of $\V$ for $\Delta U = \Delta V\,P_t$) leave the price unchanged: $P_{t+1} = P_t$.
\end{theorem}
